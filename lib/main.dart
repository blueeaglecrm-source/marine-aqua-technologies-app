      ...
      child: ListView(
        ...
        children: [
          ...
          Container(
            ...
            child: const Column(
              ...
              children: [
                ...
                SizedBox(height: 8),
                Text(
                  'Aquaculture tools made for farmers.',
          ...
          _card(
            ...
            Icons.menu_book,
            'Pond Diary',
            'Record daily pond observations',
            const PondDiaryPage(),
          ),
          _card(
            context,
    ...
    return Card(
      ...
      child: ListTile(
        ...
        leading: CircleAvatar(
          ...
          child: Icon(icon, color: marineTeal),
        ),
        title: Text(
          title,
      ...
      body: ponds.isEmpty
          ? Center(
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  const Icon(Icons.water, size: 70, color: marineTeal),
                  const SizedBox(height: 12),
                  const Text('No ponds added yet'),
                  const SizedBox(height: 12),
                  ElevatedButton.icon(
                    onPressed: addPond,
                    icon: const Icon(Icons.add),
              ...
              itemBuilder: (_, i) => Card(
                ...
                child: ListTile(
                  leading: const CircleAvatar(
                    backgroundColor: lightAqua,
                    child: Icon(Icons.water, color: marineTeal),
                  ),
                  title: Text(
                    ponds[i]['name']!,
                    style: const TextStyle(fontWeight: FontWeight.bold),
                  ),
                  subtitle: Text('${ponds[i]['area'] ?? ''} acres'),
                ),
              ),
            ),
    );
        ...
        children: [
          TextField(
            ...
            decoration: InputDecoration(
              labelText: "Today's pond observation",
            ...
            child: ElevatedButton(
              onPressed: () {
                if (note.text.trim().isNotEmpty) {
                  setState(() {
                    entries.insert(0, note.text.trim());
                    note.clear();
                  });
                }
              },
          ...
          ...entries.map(
            (e) => Card(
              color: Colors.white,
              child: ListTile(
                leading: const Icon(Icons.note_alt, color: marineTeal),
                title: Text(e),
              ),
            ),
          ),
...
class ToolsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Farmer Tools')),
      body: ListView(
        padding: const EdgeInsets.all(16),
        children: [
          _tool(context, Icons.water_drop, 'My Ponds', 'Manage ponds', const MyPondsPage()),
          _tool(context, Icons.menu_book, 'Pond Diary', 'Daily records', const PondDiaryPage()),
          _tool(context, Icons.science, 'Water Test', 'Water parameters', const WaterTestPage()),
          _tool(context, Icons.calculate, 'Biomass Calculator', 'Shrimp biomass', const BiomassPage()),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView(
        padding: const EdgeInsets.all(18),
        children: [
          _field(ph, 'pH', Icons.science),
          _field(doValue, 'DO (mg/L)', Icons.air),
          _field(salinity, 'Salinity', Icons.water_drop),
          _field(temperature, 'Temperature °C', Icons.thermostat),
          _field(ammonia, 'Ammonia', Icons.warning_amber),
          const SizedBox(height: 8),
          SizedBox(
            height: 52,
            child: ElevatedButton(
              onPressed: () {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Water test saved locally ✓')),
                );
              },
      ...
      body: ListView(
        ...
        children: [
          ...
          const Text(
            'Smart Aquaculture. Better Results.',
            textAlign: TextAlign.center,
            style: TextStyle(color: marineTeal),
          ),
          const SizedBox(height: 25),
          const Card(
            color: Colors.white,
            child: ListTile(
              leading: Icon(Icons.agriculture, color: marineTeal),
